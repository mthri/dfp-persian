...
برای متوقف کردن پیمانه، کنترل و سی را هم زمان بفشارید و دستور `docker-composer down` را اجرا کنید. پیمانه های داکر حجم زیادی از مموری را میگیرد، پس ایده خوبی هست که آنها را متوقف کنیم. آنها بی تابعیت هستند و به همین هست که ما از `volumes` برای کپی کردن کد ها روی محیطی که میتوان استفاده کرد، استفاده کردیم.
