...
برای تابع `test_create_superuser` نیز روند بالا را با یک تفاوت جزئی تکرار میکنیم. در اینجا برای ساخت کاربر از تابع `create_superuser` به جای تابع `create_user`  استفاده میشود. تفاوت این دو کاربر در این است که در ابرکاربر هر دو متغیر `is_staff` و `is_superuser` مقدار True میگیرند.
